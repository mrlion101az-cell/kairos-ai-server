...
def direct_event(context: DirectorContext, execute: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        if action not in {"ignore", "observe", "increase_threat"} and context.player:
            if not _cooldown_ready(last_player_major_action_time, context.player, DIRECTOR_MAJOR_ACTION_COOLDOWN_SECONDS):
                action = "observe"
        ...
        if execute:
            ...
            if context.player:
                ...
                if decision.action not in {"ignore", "observe", "increase_threat"}:
