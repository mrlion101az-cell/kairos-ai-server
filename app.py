...
def chat():
    try:
        ...
        # ----------------------------------------------------
        # NPC_TRIGGER messages arriving through the normal
        # Minecraft /chat endpoint must bypass Director chat
        # routing and go directly to the Command Bridge.
        # ----------------------------------------------------
        if (
            source == "minecraft"
            and message.startswith("NPC_TRIGGER")
        ):
            response = call_command_bridge(
                message=message,
                player=player,
